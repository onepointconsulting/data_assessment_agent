...
CREATE DATABASE data_assessment_questionnaire WITH OWNER = postgres ENCODING = 'UTF8' LOCALE_PROVIDER = 'libc' CONNECTION
LIMIT
    = -1 IS_TEMPLATE = False;

-- For older versions of Postgres
CREATE DATABASE data_assessment_questionnaire WITH OWNER = postgres ENCODING = 'UTF8' CONNECTION
LIMIT
    = -1 IS_TEMPLATE = False;
...
CREATE TABLE public.tb_question (
    ...
    scored boolean,
    PRIMARY KEY (id),
    CONSTRAINT topic_id FOREIGN KEY (topic_id) REFERENCES public.tb_topic (id) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE NO ACTION NOT VALID
);
...
CREATE TABLE public.tb_suggested_response (
    ...
    score integer NOT NULL,
    PRIMARY KEY (id),
    CONSTRAINT question_id FOREIGN KEY (question_id) REFERENCES public.tb_question (id) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE NO ACTION NOT VALID
);

ALTER TABLE
    public.tb_suggested_response
ALTER COLUMN
    score
SET
    DEFAULT 0;
...
CREATE TABLE public.tb_questionnaire_status (
    ...
    CONSTRAINT sentiment_id FOREIGN KEY (sentiment_id) REFERENCES public.tb_sentiment_score (id) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE NO ACTION NOT VALID
);

INSERT INTO
    public.tb_sentiment_score(id, name)
VALUES
(0, 'unknown');
...
CREATE TABLE public.tb_selected_topics (
    ...
    created_at TIMESTAMP DEFAULT NOW(),
    CONSTRAINT tb_selected_topics_topic_id FOREIGN KEY (topic_id) REFERENCES public.tb_topic (id) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE NO ACTION NOT VALID
);

ALTER TABLE
    tb_selected_topics
ADD
    CONSTRAINT session_id_topic_id_unique UNIQUE (session_id, topic_id);
...
ALTER TABLE
    tb_quiz_mode
ADD
    CONSTRAINT quiz_mode_name_unique UNIQUE (name);

insert into
    public.tb_quiz_mode(name, question_count)
values
('Easy', 3);
...
CREATE TABLE public.tb_selected_quiz_mode (
    ...
    PRIMARY KEY (id),
    CONSTRAINT tb_quiz_mode_id FOREIGN KEY (quiz_mode_id) REFERENCES public.tb_quiz_mode (id) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE NO ACTION NOT VALID
);

ALTER TABLE
    tb_selected_quiz_mode
ADD
    CONSTRAINT session_id_quiz_mode_id_unique UNIQUE (session_id, quiz_mode_id);
...
ALTER TABLE
    tb_configuration
ADD
    CONSTRAINT config_key_unique UNIQUE (config_key);

INSERT INTO
    public.tb_configuration(config_key, config_value)
VALUES
('minimum topics', 3);
...
SELECT
    ...
    (
        SELECT
            GREATEST(
                qs.affirmative_score,
                qs.undecided_score,
                qs.negative_score
            ) AS "greatest"
        FROM
            tb_question_score qs
        WHERE
            qs.question_id = q.id
    ) AS max_score,
    ...
    s.updated_at
FROM
    tb_questionnaire_status s
    JOIN tb_sentiment_score ss ON ss.id = s.sentiment_id
    JOIN tb_question q ON q.question :: text = s.question :: text FULL
    JOIN tb_topic t ON t.id = q.topic_id
    AND t.name :: text = s.topic :: text
WHERE
    q.scored = true;
